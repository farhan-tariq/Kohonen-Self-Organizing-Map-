...
public class BMUController extends VisualizationBaseController {
    ...
    private void startButtonAction() {
        startButton.setOnAction(t -> {
            initializeGridBMU(getWidth(), getHeight(), dataObject);
            inputVectors = dataObject.getConvertedInputVectorsFrom2DArray();
            if (getLayer() == null) {
                Layer layer = new Layer(getWidth(), getHeight(), 784);
                getTrainer().setTraining(layer, inputVectors, bmuGrid, getIterations());
            } else {
                getTrainer().setTraining(getLayer(), inputVectors, bmuGrid, getIterations());
            }
            start(getTrainer());
        });
    }
